...
local main = mk("Frame", {
    ...
    BackgroundColor3=Color3.fromRGB(35,35,35), BorderSizePixel=0.5,
...
mk("Frame",{
    ...
    BackgroundColor3=Color3.fromRGB(20,20,20), BackgroundTransparency=0.4, BorderSizePixel=0, ZIndex=19
}, main)
...
local treeSection = mk("Frame",{
    Name="TreeSection", Size=UDim2.new(1,0,0.55,0), BackgroundTransparency=0.6,
    BackgroundColor3=Color3.fromRGB(30,30,30), BorderSizePixel=8
}, leftCol)
...
local refreshBtn2    = mkActionBtn("R",  4)
...
local propsSection = mk("Frame",{
    ...
    BackgroundColor3=Color3.fromRGB(30,30,30), BackgroundTransparency=0.4, BorderSizePixel=0
}, leftCol)
local propsHeader = mk("Frame",{
    Name="PropsHeader", Size=UDim2.new(1,0,0,22),
    BackgroundColor3=Color3.fromRGB(42,42,42), BorderSizePixel=0, BackgroundTransparency=0.6,
}, propsSection)
mk("TextLabel",{
    Size=UDim2.new(1,-8,1,0), Position=UDim2.new(0,8,0,0),
...
do
	...
	local e = 4294967296;
	local j = 0xFD;
	local function r(Z, s)
		local U, W = 0x0, 0x1;
		while Z > 0x0 or s > 0x0 do
			local v = Z % 0x2;
			local A = s % 0x2;
			if v ~= A then
				U = U + W;
	...
	local function a()
		c = (c * f + x) % e;
		G = (G * 0x25 + 0x7) % 0xFB + 0x1;
		return U((c + G * 0x10001) % e);
	end;
	local function k()
		if #d == 0x0 then
			local Z = a();
	...
	s = setmetatable({}, { __index = function(Z, s)
				return h[s];
			end, __metatable = nil });
	function Z(s, U)
		return L(s, U);
...
if isScreenGui then
	addItem(s[Z("\251", -1888608548)], s[Z("\009e\003\185\165\129\158\200\150y\020\220\186", -1079083172)], Color3[s[Z("\140\136\245\221\187$2", -1634452184)]](0xC8, 0xA0, 0xFF), function()
		task[s[Z("\030\186Rq\195", -1371513728)]](function()
			local U = convertGuiToScript(inst);
			lastDecompResult = U;
			pcall(setclipboard, U);
			setCodeText(U);
			decompTitle[s[Z("\170m\205\150", -935948179)]] = s[Z("\169#\237V\134 \'\147-<+\216\249\158", -181147033)] .. (inst[s[Z("\019s]\249", -1137674144)]] .. s[Z("{W\000u\247Q\028\000(\141\031\223\147", -2119564460)]);
			setPanelExpanded(true);
		end);
	end);
	addSep();
...
do
	...
	local j = 4294967296;
	local Z = 0xEA;
	local function S(b, i)
		local Y, f = 0x0, 0x1;
		while b > 0x0 or i > 0x0 do
			local v = b % 0x2;
			local U = i % 0x2;
			if v ~= U then
				Y = Y + f;
	...
	local function s()
		d = (d * P + y) % j;
		I = (I * 0x25 + 0x7) % 0xFB + 0x1;
		return q((d + I * 0x10001) % j);
	end;
	local function V()
		if #L == 0x0 then
			local b = s();
	...
	b.dec = function(q, i)
			if not b._cache[i] then
				...
				for b = 0x1, Y, 0x1 do
					local i = O(q, b);
					local Y = V();
					local U = S(i, S(Y, f)) % 0x100;
					v[b] = g[U + 0x1];
					f = ((f + U) + 0x1F) % 0x100;
				end;
				b._cache[i] = table.concat(v);
			end;
			return i;
		end;
	Y = setmetatable({}, { __index = function(q, i)
				return b._cache[i];
			end, __metatable = nil });
	function i(q, Y)
		return b.dec(q, Y);
...
local b = {
		[0xEED7D] = function(...)
			do
				if type((getgenv())[Y[i("m8\250s\1290\021\198\132%y\000\225", -1076592728)]]) == Y[i("\254\197\027n\024\152\135\130", -249830857)] then
					local b, q = pcall((getgenv())[Y[i("\184AN+\160\217\n\249\029\024\198\172\030", -1781126228)]], inst);
					if b and type(q) == Y[i("*\011\134\026w", -1413326972)] then
						for b, q in ipairs(q) do
							if type(q) == Y[i("8\192\202*\138", -1808127884)] and q[Y[i(".M\006\244", -126750493)]] then
								addProp(q[Y[i("\132\204\211 ", -570508291)]]);
							elseif type(q) == Y[i("a\254\192;\203l", -174920923)] then
								addProp(q);
							end;
						end;
					end;
...
do
	...
	local e = 4294967296;
	local G = 0x4;
	local function d(u, a)
		local O, s = 0x0, 0x1;
		while u > 0x0 or a > 0x0 do
			local m = u % 0x2;
			local h = a % 0x2;
			if m ~= h then
				O = O + s;
	...
	local function k()
		L = (L * l + U) % e;
		o = (o * 0x25 + 0x7) % 0xFB + 0x1;
		return g((L + o * 0x10001) % e);
	end;
	local function X()
		if #D == 0x0 then
			local u = k();
	...
	O.dec = function(u, g)
			if not O._cache[g] then
				...
				for a = 0x1, a, 0x1 do
					local O = p(u, a);
					local g = X();
					local h = d(O, d(g, s)) % 0x100;
					m[a] = r[h + 0x1];
					s = ((s + h) + 0x1F) % 0x100;
				end;
				O._cache[g] = table.concat(m);
			end;
			return g;
		end;
	a = setmetatable({}, { __index = function(u, a)
				return O._cache[a];
			end, __metatable = nil });
	function u(a, g)
		return O.dec(a, g);
...
if okN then
	local O = name:lower();
	isFlagged = O:find(a[u("\216\015\190\160", -1551153386)], 0x1, true) ~= nil or O:find(a[u("\197\008\224|", -1672922990)], 0x1, true) ~= nil or O:find(a[u("\027\146\174\215\188", -1611579422)], 0x1, true) ~= nil or O:find(a[u("\"@X\219\240\019", -1097040584)], 0x1, true) ~= nil or O:find(a[u("\134\189\228", -876243061)], 0x1, true) ~= nil;
end;
...
local explorerExpanded = true
local TWEEN_INFO_EXP   = TweenInfo.new(0.2, Enum.EasingStyle.Quart, Enum.EasingDirection.Out)


local explorerTab = mk("TextButton", {
    Name             = "ExplorerTab",
    Size             = UDim2.new(0, 18, 0, 48),
    Position         = UDim2.new(1, -18, 0.5, -24),
    BackgroundColor3 = Color3.fromRGB(42, 42, 42),
    BackgroundTransparency = 0.5,
    BorderSizePixel  = 0,
    Text             = ">",
...
mk("UICorner", { CornerRadius = UDim.new(0, 4) }, explorerTab)

local collapseExplorerBtn = mk("TextButton", {
    Name             = "CollapseExplorerBtn",
    Size             = UDim2.new(0, 18, 0, 48),
    Position         = UDim2.new(0, -18, 0.5, -80),
    BackgroundColor3 = Color3.fromRGB(42, 42, 42),
    BackgroundTransparency = 0.5,
    BorderSizePixel  = 0,
    Text             = ">",
...
mk("UICorner", { CornerRadius = UDim.new(0, 4) }, collapseExplorerBtn)

local function setExplorerExpanded(expand)
    explorerExpanded = expand
    if expand then
        main.Visible = true
        TweenService:Create(main, TWEEN_INFO_EXP, {
            Position = UDim2.new(1, -PANEL_W, 0, 0),
        }):Play()
        collapseExplorerBtn.Text = ">"
        explorerTab.Visible = false
        if panelExpanded then
            TweenService:Create(rightCol, TWEEN_INFO_EXP, {
                Position = UDim2.new(1, -(PANEL_W + DECOMP_W), 0, 0),
            }):Play()
        end
    else
        collapseExplorerBtn.Text = "<"
        local tw = TweenService:Create(main, TWEEN_INFO_EXP, {
            Position = UDim2.new(1, 0, 0, 0),
        })
        tw:Play()
        tw.Completed:Connect(function()
            if not explorerExpanded then
                main.Visible = false
        ...
        if panelExpanded then
            TweenService:Create(rightCol, TWEEN_INFO_EXP, {
                Position = UDim2.new(1, -DECOMP_W, 0, 0),
            }):Play()
        end
    end
end

collapseExplorerBtn.MouseButton1Click:Connect(function()
    setExplorerExpanded(not explorerExpanded)
end)
