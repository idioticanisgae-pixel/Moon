...
class LuaIDE(QMainWindow):
    ...
    def create_new_tab(self, title):
        ...
        editor = CodeEditor()
        t = get_theme()
        editor.setFont(QFont("Consolas", self._current_font_size))
        editor.setStyleSheet(f"""
            QPlainTextEdit {{
                background-color: {t['editor_bg']};
                color: {t['editor_fg']};
                border: none;
                selection-background-color: {t['editor_sel']};
            }}
        """)
    ...
    def apply_theme(self, theme_name: str):
        ...
        for i in range(self.tab_widget.count()):
            ...
            if editor:
                ...
                """)
                # Force line number area repaint
                editor.line_number_area.update()
                editor.highlight_current_line()
                # Rebuild syntax highlighter colors
                if hasattr(editor, 'highlighter'):
                    editor.highlighter.rehighlight_with_theme()
...
''',
    ...
    def format_current_code(self):
        ...
        # Show options dialog (remember last-used options)
        ...
        options = dialog.get_options()
        self._last_format_options = options   # persist for next time
