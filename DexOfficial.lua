...
local function main()
	...
	local function Decompile(bytecode, options)
			...
			if options.DecompilerMode == "disasm" then
				...
				local function writeActions(protoActions)
					...
					local function fmtConst(k)
						...
						local n = tonumber(k.value)
						if type(n) == "number" then
							-- If the value is a whole number, emit it without decimal noise
							if n == math.floor(n) and math.abs(n) < 1e15 then
								return string.format("%d", n)
							end
							return tostring(tonumber(string.format("%0."..options.ReaderFloatPrecision.."f", n)))
						end
		...
		local function manager(proceed, issue)
			if proceed then
				...
				-- Check timeout BEFORE inspecting res so a real error isn't
				-- silently swallowed just because the clock also overran.
				if elapsed >= options.DecompilerTimeout then
	...
	local function _coImpl(text)
		...
		for _, line in ipairs(pass3) do
			-- detect multi-assign: v1, v2, ... = rhs  (Lua pattern, no (?:...) POSIX)
			local indent_part = line:match("^(%s*)")
			local body = line:sub(#indent_part + 1)
			-- check if line starts with at least two vN registers separated by commas
			local is_multi = body:match("^v%d+%s*,%s*v%d+") ~= nil
			local regs_str, rhs
			if is_multi then
				regs_str, rhs = body:match("^(v%d+%s*(?:,%s*v%d+)*)%s*=(.+)$")
				-- fallback using plain Lua pattern without (?:...)
				if not regs_str then
					local eq_pos = body:find("%s*=")
					if eq_pos then
						local lhs_candidate = body:sub(1, eq_pos - 1)
						rhs = body:sub(eq_pos + 1)
						-- validate lhs is purely comma-separated vN tokens
						if lhs_candidate:match("^%s*v%d+%s*(?:,%s*v%d+%s*)*$") or
						   lhs_candidate:match("^v%d+") then
							-- Lua-safe check: split on commas and verify each token
							local all_regs = true
							for tok in lhs_candidate:gmatch("[^,]+") do
								if not trim(tok):match("^v%d+$") then all_regs = false; break end
							end
							if all_regs and lhs_candidate:find(",") then
								regs_str = lhs_candidate
							end
						end
					end
				end
			...
			if regs_str then
				...
				if any_new then
					line = indent_part .. "local " .. trim(new_regs) .. " =" .. rhs
				end
			else
	...
	ScriptViewer.ViewScript = function(scr)
		...
		if ZukDecompile and okBC and bytecode and bytecode ~= "" then
			...
			-- Reject timeout/empty strings so we fall through to the error message
			-- instead of displaying a useless "-- DECOMPILER TIMEOUT" in the viewer.
			...
			if isValidResult then
				...
				local ppOk, ppRes = pcall(function() return cleanOutput(prettyPrint(result)) end)
				source = source..(ppOk and ppRes or result)
				PreviousScr = scr
				dumpbtn.TextColor3 = Color3.new(1,1,1)
			elseif okD and result and result:find("^%-%- DECOMPILER TIMEOUT") then
				-- surface the timeout explicitly instead of silently falling through
				source = "-- Script Path: "..getPath(scr).."\n"
